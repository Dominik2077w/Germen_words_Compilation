...
## 使用步骤
- 1 .使用者应该按照如图结构在python项目目录下构建自己的文件夹结构，并将资料文件(md/非扫描版pdf/txt)放入对应文件夹!
...
## 碎碎念
- 为什么只接受非扫描版pdf文件和markdown文件或txt？因为我们在课上用的是小米自带的同声传译，导出后的字幕文件是markdown。以及我们的Moodle课件都是非扫描版pdf。如果你有别的需求，可以自己增加几个读取文档的函数。
